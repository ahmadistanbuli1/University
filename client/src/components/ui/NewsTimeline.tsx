  ...
  return (
    <ol className={cn('relative m-0 list-none space-y-0 p-0', className)}>
      {items.map((item, index) => {
        const excerpt = item.summary ?? item.content?.slice(0, 220) ?? '';
        return (
          <li key={item.id} className="relative pb-10 ps-8 sm:ps-10 last:pb-0">
            {index < items.length - 1 ? (
              <span
                className="absolute start-[0.6875rem] top-4 bottom-0 w-0.5 bg-gradient-to-b from-brand-light/80 to-brand/20 dark:from-brand-light/60 dark:to-transparent"
                aria-hidden
              />
            ) : null}
            <span
              className="absolute start-0 top-1.5 grid size-6 place-items-center rounded-full bg-gradient-to-br from-brand to-brand-light text-[10px] font-black text-white shadow-md ring-4 ring-white dark:ring-zinc-900"
              aria-hidden
            >
              {index + 1}
            </span>
            <Card className="overflow-hidden border-zinc-200/90 p-0 transition hover:border-brand/30 hover:shadow-lg dark:border-white/10">
              ...
              <article className="p-5 sm:p-6">
                ...
                <Link
                  to={`/news/${item.id}`}
                  className="mt-4 inline-block text-sm font-bold text-brand dark:text-brand-light"
                >
